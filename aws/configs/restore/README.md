...
This folder contains templates and configurations for Velero restore operations on AWS EKS.
...
## Usage:
This folder is used when setting up restore operations on a target EKS cluster, allowing you to restore backups created from a source cluster. The priority class ensures stable restore operations even when cluster resources are constrained during the restore process.

## Restore Process:
1. Credentials and IAM roles must be configured for Velero access
